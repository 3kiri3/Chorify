...
import { getData, saveData, addChoreToSchedule, transferChore, addLog, getSchedule } from './storage';
...
function App() {
  ...
  const [scannedChore, setScannedChore] = useState(null);

  if (!currentUser) {
  return <Login onLogin={(user) => setCurrentUser(user)} />;
}
const handleScan = (choreId) => {
  const data = getData();
...
if (view === 'tictactoe') {
  ...
  <div className="w-full max-w-md bg-gradient-to-br from-[#1F1F1F] to-[#131313] rounded-3xl p-8 shadow-2xl border border-[#B5CF50]/20">
    <h2 className="text-white text-2xl font-lalezar font-bold mb-2 text-center">Pick a day</h2>
    <p className="text-[#B5CF50] text-sm text-center mb-6">When do you want to do {scannedChore?.name}?</p>
    <div className="flex flex-col gap-3">
      {days.map((day) => (
        <button
          key={day}
          onClick={() => {
            addChoreToSchedule(currentUser.id, day, scannedChore.name);
            setScannedChore(null);
            setScanStep('default');
            setView('success');
          }}
          className="w-full py-3 rounded-2xl border border-[#B5CF50]/30 text-white font-semibold hover:border-[#B5CF50] hover:bg-[#B5CF50]/10 transition-all duration-300"
        >
          {day}
        </button>
      ))}
  ...
  <div className="w-full max-w-md bg-gradient-to-br from-[#1F1F1F] to-[#131313] rounded-3xl p-8 shadow-2xl border border-[#B5CF50]/20">
    <h2 className="text-white text-2xl font-lalezar font-bold mb-2 text-center">Chore Found!</h2>
    <p className="text-[#B5CF50] text-center text-lg mb-2">{scannedChore.name}</p>
    <p className="text-white/40 text-center text-sm mb-8">
      {scannedChore.owner && scannedChore.owner !== currentUser.id
        ? `This belongs to someone else — challenge them?`
        : `Add this to your schedule?`}
    </p>
    <div className="flex flex-col gap-4">
      {scannedChore.owner && scannedChore.owner !== currentUser.id ? (
        <button
          onClick={() => { setScanStep('challenge-options'); setScannedChore(null); }}
          className="w-full py-4 bg-gradient-to-r from-[#B5CF50] to-[#a8c43a] text-black rounded-2xl font-bold text-lg"
        >
          Challenge! 🎮
        </button>
      ) : (
        <button
          onClick={() => { setScanStep('pick-day'); }}
          className="w-full py-4 bg-gradient-to-r from-[#B5CF50] to-[#a8c43a] text-black rounded-2xl font-bold text-lg"
        >
          Add to schedule 📅
        </button>
  ...
  <div className="w-72 h-72 bg-gradient-to-br from-[#131313] to-[#1F1F1F] cursor-pointer rounded-3xl flex items-center justify-center border-2 border-[#B5CF50]/30 hover:border-[#B5CF50] hover:shadow-2xl hover:shadow-[#B5CF50]/30 transition-all duration-300"
  ...
  <button
    ...
    className="px-3 py-1 rounded-full text-xs font-semibold border border-[#B5CF50]/40 text-[#B5CF50] hover:bg-[#B5CF50]/10 transition-all duration-300"
  >
    switch account
  </button>
</div>
      ...
      <div className='px-6'>
        <div className='flex flex-col gap-2'>
          ...
          <h2 className="text-4xl font-lalezar font-bold text-white">{currentUser.name}</h2>
